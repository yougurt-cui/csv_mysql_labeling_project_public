...
# -*- coding: utf-8 -*-
"""
将 douyin_raw_comments_pre_info_flat 表中 product_entities 含“洗碗机”的实体
对应 task_type 统一改为“洗碗机收纳”。

仅修改命中的实体；其他标签保持不变。支持 --dry-run / --limit。
"""
...
def mark_dishwasher(tags: List[Dict[str, Any]]) -> bool:
    changed = False
    for item in tags:
...
def fetch_rows(conn, table: str, limit: int | None) -> List[tuple]:
    sql = f"""
        ...
        FROM {table}
        WHERE JSON_SEARCH(product_entities, 'one', '%%{KEYWORD}%%') IS NOT NULL
...
def main():
    ap = argparse.ArgumentParser(description="将含洗碗机的 task_type 置为 洗碗机收纳")
    ...
    try:
        ...
        for pre_product_id, ents_raw, tags_raw in rows:
            ...
            changed = mark_dishwasher(tags)
            after = json.dumps(tags, ensure_ascii=False, sort_keys=True)
            if not changed or before == after:
                continue
            updated_rows += 1
            updated_entities += sum(1 for t in tags if KEYWORD in t.get("entity", "") and t.get("task_type") == TARGET_TASK)
            if args.dry_run:
                continue
